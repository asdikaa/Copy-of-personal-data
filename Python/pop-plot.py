...
def draw_line(x1, y1, x2, y2 ,tp=1):
    g.setcell(x1, y1, 1)
...
xl = g.getstring("Enter the length of x axis:",str(xlenth), "Population plotter")
xlen = int(xl)        # length of x axis
yl = g.getstring("Enter the length of y axis:",str(ylenth), "Population plotter")
ylen = int(yl)        # length of y axis
numsteps = xl
s = g.getstring("Enter the number of steps:",str(numsteps), "Population plotter")
...
# use RuleLoader with an unbounded grid
g.setalgo(algo)
g.setrule(rule)
#g.setcolors(all_color)
...
draw_line(xlen, 0,xlen, -ylen)
draw_line(0, -ylen, xlen, -ylen)
...
for i in range(numsteps):
    ...
    draw_line(x, -y, newx, -newy,2)
    g.setcell(x, -y, 1)
    g.setcell(newx, -newy,1)
    x = newx
    y = newy
    newsecs = time()
